...
-behaviour(swag_privdoc_server_logic_handler).
...
-spec authorize_api_key(swag_privdoc_server:operation_id(), swag_privdoc_server:api_key()) ->
...
-type processing_context() :: #{
    swagger_context := swag_privdoc_server:request_context(),
    woody_context   := woody_context:ctx()
}.

-spec handle_request(
    OperationID :: swag_privdoc_server:operation_id(),
    Req :: request_data(),
    SwagContext :: swag_privdoc_server:request_context()
) ->
    {ok | error, swag_privdoc_server_logic_handler:response()}.
