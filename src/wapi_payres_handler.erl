...
-behaviour(swag_payres_server_logic_handler).
...
-spec authorize_api_key(swag_payres_server:operation_id(), swag_payres_server:api_key()) ->
...
-type processing_context() :: #{
    swagger_context := swag_payres_server:request_context(),
    woody_context   := woody_context:ctx()
}.
-type request_result() :: {ok | error, swag_payres_server_logic_handler:response()}.

-define(request_result, wapi_req_result).

-spec handle_request(
    OperationID :: swag_payres_server:operation_id(),
    Req :: request_data(),
    SwagContext :: swag_payres_server:request_context()
) ->
    request_result().
...
process_request('StoreBankCard', Req, Context) ->
    ...
    {ok, {200, [], maps:merge(to_swag(CardData), to_swag(AuthData))}};
process_request('GetBankCard', Req, Context) ->
    {ok, {200, [], to_swag(get_card_data_from_cds(Req, Context))}}.
...
put_card_data_to_cds(CardData, SessionData, Context) ->
    ...
    case service_call(Call, Context) of
        ...
        {exception, Exception} ->
            case Exception of
                #'InvalidCardData'{} ->
                    throw_result({ok, {400, [], logic_error(invalidRequest, <<"Card data is invalid">>)}});
                #'KeyringLocked'{} ->
                    % TODO
                    ...
                    throw_result({error, reply_5xx(503)})
            end
    end.

get_card_data_from_cds(Token, Context) ->
    Call = {cds_storage, 'GetCardData', [Token]},
    case service_call(Call, Context) of
        {ok, CardData = #'CardData'{}} ->
            {bank_card, CardData};
        {exception, #'CardDataNotFound'{}} ->
            throw_result({ok, {404, [], general_error(<<"Invoice not found">>)}})
    end.
